...
class Scorpion:
    ...
    def __load_metadata(self):
        try:
            metadata = pyexiv2.Image(self.path)
            try:
            ...
            print(f"Scorpion: [ ERROR ] __load_metadata({self.path}): pyexiv2.Image({self.path}): {e}")
    ...
    def modify_metadata(self, key: str, value: str | None):
        try:
            ...
            print(f"Scorpion: [ ERROR ] modify_metadata({key}, {value}): pyexiv2.Image({self.path}): {e}")

    def __update_metadata(self, key: str, value: str):
        metadata = pyexiv2.metadata.ImageMetadata(self.path)
        metadata.read()
        try:
            if not value:
                del metadata[key]
